...
# FC7 EMBEDDINGS (4096D)
APE_TRAIN_DIR = WORKSPACE / "embeddings_apemorph_train_fc7"
APE_TEST_DIR  = WORKSPACE / "embeddings_apemorph_test_fc7"
FRLL_DIR      = WORKSPACE / "embeddings_frll_fc7"

APE_TRAIN_X = np.load(APE_TRAIN_DIR / "embeddings_4096.npy")
APE_TRAIN_Y = np.load(APE_TRAIN_DIR / "labels.npy")

APE_TEST_X  = np.load(APE_TEST_DIR / "embeddings_4096.npy")
APE_TEST_Y  = np.load(APE_TEST_DIR / "labels.npy")

FRLL_X = np.load(FRLL_DIR / "embeddings_4096.npy")
FRLL_Y = np.load(FRLL_DIR / "labels.npy")

OUT_DIR = WORKSPACE / "tsne_results"
OUT_DIR.mkdir(exist_ok=True)

OUT_FIG = OUT_DIR / "tsne_FC7_APE_train_test_FRLL_6groups.png"
...
# ======================================================
X = np.vstack([APE_TRAIN_X, APE_TEST_X, FRLL_X]).astype(np.float32)
...
plt.title(
    "t-SNE visualization of VGG19 FC7 embeddings (4096D)\n"
...
print("t-SNE FC7 saved to:", OUT_FIG)
